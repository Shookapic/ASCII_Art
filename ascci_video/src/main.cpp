 ...
 * 
 * This function checks if the number of command line arguments is equal to 2. If not, it displays an error message and exits the program.
 * 
...
void check_args(int ac, char **av)
{
    if (ac != 2)
    {
        std::cerr << "Usage: " << av[0] << " <video_file>" << std::endl;
        exit(1);
    }
}
...
int main(int ac, char **av)
{
    ...
    check_args(ac, av);
    video = cv::VideoCapture(av[1]);
    if (!video.isOpened())
    {
        std::cerr << "Error: Could not open video file " << av[1] << std::endl;
        exit(1);
    }
